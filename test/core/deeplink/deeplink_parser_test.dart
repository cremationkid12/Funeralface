...
void main() {
  ...
  test('extracts token from query parameter', () {
    ...
    expect(extractFamilyAssignmentToken(uri, expectedHost: 'links.example.com'), 'xyz');
  });
  ...
  test('path token wins over query when both present', () {
    ...
    expect(extractFamilyAssignmentToken(uri, expectedHost: 'links.example.com'), 'path-token');
  });
}
